...
class MIDOG2021Evaluation():
    ...
    def __init__(self,path):
        ...
        # ADDED
        self._predictions_test_file = os.path.join(path, 'mitotic-figures-test.json')
    ...
    def load_predictions(self):
      ...
      predictions_test_json = json.load(open(self._predictions_test_file, 'r'))
      predictions = {}
      ...
      for fname, pred in predictions_test_json.items():
          # Process test predictions similarly to regular predictions
          if fname not in self.gt:
              print('Warning: Found test predictions for image', fname, 'which is not part of the ground truth.')
              continue
  
          if 'points' not in pred:
              print('Warning: Wrong format. Field points is not part of test detections.')
              continue
          ...
          for point in pred['points']:
              ...
              if 'name' not in point:
                  print('Warning: Old format. Field name is not part of test detections.')
  
              if 'probability' not in point:
                  print('Warning: Old format. Field probability is not part of test detections.')
  
              if 'point' not in point:
                  print('Warning: Point is not part of test points structure.')
                  continue
  
              points.append([*point['point'][0:3], detected_class, detected_thr])
  
          # Add test predictions to the existing predictions dictionary
    ...
    def score(self, val=True, det=0.5):
        ...
        for idx, case in enumerate(cases):
            ...
            pred_dict = [{'boxes': Tensor([[x-bbox_size,y-bbox_size, x+bbox_size, y+bbox_size] for (x,y,z,_,_) in self.predictions[case]]).to('cuda'), 
                         'labels': Tensor([1,]*len(self.predictions[case])).to('cuda'),
                         'scores': Tensor([sc for (x,y,z,sc,_) in self.predictions[case]]).to('cuda')}]
            target_dict = [{'boxes': Tensor([[x-bbox_size,y-bbox_size, x+bbox_size, y+bbox_size] for (x,y,z) in self.gt[case]]).to('cuda'),
                           'labels' : Tensor([1,]*len(self.gt[case])).to('cuda')}]
