...
import json
import torch
from training_torch_RetinaNet import initialize_retinanet_original
import time
...
class MyMitosisDetection:
    def __init__(self, path, config, detect_threshold, nms_threshold):
        ...
        self.model = initialize_retinanet_original(num_classes = 2)
...
def inference_RetinaNet(directory):
    for root, dirs, files in os.walk(directory):
        for dir in dirs:
            ...
            start_time = time.time()  # Registering the start time
            ...
            end_time = time.time()  # Registering the end time
