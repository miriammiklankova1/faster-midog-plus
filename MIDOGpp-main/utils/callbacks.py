...
class BBLossMetrics(LearnerCallback):
    ...
    def on_batch_end(self, last_target, train, **kwargs):
        ...
        #self.nums += bs
        for name in self.names:
          self.metrics[name] += self.learn.loss_func.metrics[name].sum().item()  # Accumulate total metric
        self.nums += last_target[0].size(0)  # Accumulate total count
...
class BBMetrics_FasterRCNN(LearnerCallback):
    "Add `loss_func.metrics` to metrics named by `loss_func.metric_names`"
    _order = -20 #Needs to run before the recorder

    def on_train_begin(self, **kwargs):
        "Add the metrics names to the `Recorder`."
        self.names = getattr(self.learn.loss_func, 'metric_names', [])
        print(self.names)
        if not self.names:
            ...
            if hasattr(m, 'metric_names'):
                for name in m.metric_names:
                    if name not in self.names:
                        self.names.append(name)
        if not self.names:
            warn('Metrics names requested but no metrics.metric_names provided')
        self.learn.recorder.add_metric_names(self.names)
