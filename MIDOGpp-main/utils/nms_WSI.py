...
from utils.utils_ObjectDetection import bbox_iou
import torchvision
...
# ADDED
def nms_patch_FasterRCNN(boxes, scores, thresh=0.5):
    idx_sort = scores.argsort(descending=True)
    boxes, scores = boxes[idx_sort], scores[idx_sort]
    to_keep, indexes = [], torch.arange(len(scores))

    while len(scores) > 0:
        to_keep.append(idx_sort[indexes[0]])
        iou_vals = bbox_iou(boxes, boxes[:1]).squeeze()
